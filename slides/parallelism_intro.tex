...
\begin{frame}{Ring AllReduce}
  ...
  \textbf{Key idea:} split each GPU's tensor into $N$ \emph{chunks}; move them around a ring
  so all links stay busy. Below we \emph{follow one chunk} through both phases ($N=4$).

  \vspace{0.3em}
  \begin{columns}
    \begin{column}{0.5\textwidth}
      \centering
      {\small\textbf{Phase 1: ReduceScatter}}\\[-0.1em]
      {\scriptsize following chunk 0, it moves + accumulates}

      \vspace{0.4em}
      \begin{tikzpicture}[
        tok/.style={draw, rounded corners=2pt, fill=blue!12, font=\tiny,
          inner sep=1.5pt, minimum height=0.42cm},
        gline/.style={gray!30, line width=0.3pt},
      ]
        % row guides + gpu labels
        \foreach \i in {0,1,2,3}{
          \draw[gline] (-0.1,{-\i*0.72}) -- (3.75,{-\i*0.72});
          \node[anchor=east, font=\tiny\bfseries] at (-0.2,{-\i*0.72}) {G\i};
        }
        % step labels
        \foreach \s/\lab in {0/start,1/step 1,2/step 2,3/step 3}{
          \node[font=\tiny, gray] at ({\s*1.1},0.55) {\lab};
        }
        % tokens on the diagonal (chunk 0 partial walking down the ring)
        \node[tok] (a) at (0,0)          {$a_0$};
        \node[tok] (b) at (1.1,-0.72)    {$a_0{+}b_0$};
        \node[tok] (c) at (2.2,-1.44)    {$\cdots{+}c_0$};
        \node[tok, fill=blue!35] (d) at (3.3,-2.16) {$\Sigma_0$};
        \draw[->, thick, blue!65!black] (a) -- (b) node[midway,sloped,above,font=\tiny]{$+$};
        \draw[->, thick, blue!65!black] (b) -- (c) node[midway,sloped,above,font=\tiny]{$+$};
        \draw[->, thick, blue!65!black] (c) -- (d) node[midway,sloped,above,font=\tiny]{$+$};
      \end{tikzpicture}

      {\scriptsize G3 ends up owning the full $\Sigma_0$.}
    \end{column}
    \begin{column}{0.5\textwidth}
      \centering
      {\small\textbf{Phase 2: AllGather}}\\[-0.1em]
      {\scriptsize the finished $\Sigma_0$ circulates (copy, no add)}

      \vspace{0.4em}
      \begin{tikzpicture}[
        tok/.style={draw, rounded corners=2pt, fill=green!15, font=\tiny,
          inner sep=1.5pt, minimum height=0.42cm},
        gline/.style={gray!30, line width=0.3pt},
      ]
        % rows in ring-visit order starting from owner G3
        \foreach \i/\g in {0/3,1/0,2/1,3/2}{
          \draw[gline] (-0.1,{-\i*0.72}) -- (3.75,{-\i*0.72});
          \node[anchor=east, font=\tiny\bfseries] at (-0.2,{-\i*0.72}) {G\g};
        }
        \foreach \s/\lab in {0/start,1/step 1,2/step 2,3/step 3}{
          \node[font=\tiny, gray] at ({\s*1.1},0.55) {\lab};
        }
        \node[tok, fill=green!30] (s) at (0,0)      {$\Sigma_0$};
        \node[tok] (t) at (1.1,-0.72)    {$\Sigma_0$};
        \node[tok] (u) at (2.2,-1.44)    {$\Sigma_0$};
        \node[tok] (v) at (3.3,-2.16)    {$\Sigma_0$};
        \draw[->, thick, green!45!black] (s) -- (t) node[midway,sloped,above,font=\tiny]{copy};
        \draw[->, thick, green!45!black] (t) -- (u) node[midway,sloped,above,font=\tiny]{copy};
        \draw[->, thick, green!45!black] (u) -- (v) node[midway,sloped,above,font=\tiny]{copy};
      \end{tikzpicture}

      {\scriptsize After 3 steps every GPU holds $\Sigma_0$.}
    \end{column}
  \end{columns}

  \vspace{0.3em}
  {\footnotesize All $N$ chunks flow \emph{simultaneously}, staggered by one GPU, so every link
  is busy and each GPU finishes a distinct $\Sigma_j = a_j{+}b_j{+}c_j{+}d_j$.}

  \begin{itemize}
    \item Each GPU sends/receives only $\frac{2(N-1)}{N}\,|D| \approx 2|D|$, independent of $N$: \textbf{bandwidth-optimal}
    \item \textbf{Tree vs.\ ring:} tree minimizes \emph{latency} ($O(\log N)$ steps); ring minimizes \emph{bandwidth}. Gradients are large, so bandwidth dominates and ring wins
  \end{itemize}
\end{frame}
