  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.48\textwidth}
      ...
      \begin{itemize}
        ...
        \item When scaling batch size $B \to kB$: scale $\eta \to k\eta$ (linear scaling rule)
      \end{itemize}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{Batch Size and the Linear Scaling Rule}
  ...
  \textbf{Limitations:}
  \begin{itemize}
    \item Assumes $\text{Var}[\hat{g}]$ is small enough that linear approximation holds --- breaks for very large $B$
...
\begin{frame}{Practical Hyperparameter Selection Workflow}
  ...
  \begin{enumerate}
    ...
    \item \textbf{Scale batch size with compute.} Use the linear scaling rule; start small, ramp up. Stay near $B_{\text{crit}}$.
...
\begin{frame}{Summary}
  ...
  \begin{block}{Key Takeaways}
    \begin{itemize}
      ...
      \item \textbf{Batch size} interacts with LR via the linear scaling rule; respect $B_{\text{crit}}$
