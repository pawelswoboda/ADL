...
% -----------------------------------------------------------------------
\begin{frame}{Batch Size and Learning Rate Scaling}
  ...
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      \textbf{Linear scaling}~\cite{goyal2017accurate}:
      \[
        B \to kB \;\Rightarrow\; \eta \to k\eta
      \]
      \begin{itemize}
        \item With $k\times$ larger batch, $k\times$ fewer steps per epoch
        \item Scale LR proportionally to cover the same distance in weight space
        \item Matches the \emph{expected} update: one large step $\approx$ $k$ small steps
      \end{itemize}
    \end{column}
    \begin{column}{0.48\textwidth}
      \textbf{Square root scaling}~\cite{hoffer2017train}:
      \[
        B \to kB \;\Rightarrow\; \eta \to \sqrt{k}\,\eta
      \]
      \begin{itemize}
        \item Gradient noise std decreases as $1/\sqrt{B}$
        \item Scaling by $\sqrt{k}$ preserves the signal-to-noise ratio of parameter updates
        \item More conservative; works better for very large $B$ where linear scaling diverges
      \end{itemize}
    \end{column}
  \end{columns}
  ...
  \begin{itemize}
    \item Linear scaling is the default for moderate batch size increases ($k \leq 8$--$16$); requires LR warmup
    \item Square root scaling is more robust at very large batch sizes (above $B_\text{crit}$)
    \item In practice, the optimal exponent is often between $0.5$ and $1.0$ and depends on the optimizer, model, and data
  \end{itemize}
