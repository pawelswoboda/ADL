...
\begin{frame}{Motivation: Why Attention?}
  ...
  \begin{block}{References}
    \cite{vaswani2017attention} \quad \cite{alammar2018illustrated}
  \end{block}
\end{frame}

% -----------------------------------------------------------------------
\begin{frame}{Token Embeddings}
  Each token is mapped to a learnable feature vector --- its \textbf{token embedding}.
  The embedding matrix $E \in \mathbb{R}^{V \times d}$ is a lookup table: row $i$ gives the embedding for token $i$.
  ...
  \begin{tikzpicture}[scale=0.95,
    ...
    \node[font=\small\bfseries, text=green!60!black] at (-1.18, 3.55) {$E$};
    \foreach \ri in {0,...,6} {
      \foreach \ci in {0,...,3} {
    ...
    \node[font=\tiny, text=green!60!black, anchor=east] at (-1.75, 2.90-2*0.26+0.13) {$i_1$};
    \node[font=\tiny, text=green!60!black, anchor=east] at (-1.75, 2.90-5*0.26+0.13) {$i_2$};
...
\begin{frame}{Causal Self-Attention}
  ...
  \begin{columns}[T]
    \begin{column}{0.50\textwidth}
      ...
      \]
      After softmax, $-\infty$ becomes $0$ --- future positions receive zero weight.
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.60\textwidth}
      ...
      \begin{tikzpicture}[scale=0.55,
        ...
        % ======== INPUT TOKENS (decoder) ========
        \node[font=\small\bfseries, text=violet!70!black] at (\xA, -0.40) {\textit{tok$_1$}};
        \node[font=\small\bfseries, text=violet!70!black] at (\xB, -0.40) {\textit{tok$_2$}};

        \node[font=\tiny, text=violet!70!black, anchor=east] at (\xA-0.30, 0.10) {$y_1$};
        \foreach \ci in {0,...,3} {
          \pgfmathsetmacro{\cx}{\xA - 2*\cs + \ci*\cs + \cs/2}
          \node[acell, minimum width=\cs cm, minimum height=\cs cm,
                fill=qcol!50] at (\cx, 0.10) {};
        }
        \node[font=\tiny, text=violet!70!black, anchor=east] at (\xB-0.30, 0.10) {$y_2$};
        \foreach \ci in {0,...,3} {
          \pgfmathsetmacro{\cx}{\xB - 2*\cs + \ci*\cs + \cs/2}
          \node[acell, minimum width=\cs cm, minimum height=\cs cm,
                fill=qcol!50] at (\cx, 0.10) {};
        ...
        \node[block, fill=orange!12, draw=orange!40,
              minimum width=4.8cm, text=orange!80!black]
          (csa) at (0, 1.05) {Causal Self-Attention};
        ...
        \node[block, fill=yellow!15, draw=yellow!50!black,
              minimum width=4.8cm, font=\small, text=yellow!50!black]
          (an1) at (0, 2.75) {Add \& LN};

        % Residual 1
        \draw[gray!50, thin, dashed, rounded corners=3pt]
          (\xA, 0.55) -- (-5.00, 0.55) -- (-5.00, 2.75) -- (an1.west);
        \draw[gray!50, thin, dashed, rounded corners=3pt]
          (\xB, 0.55) -- (5.00, 0.55) -- (5.00, 2.75) -- (an1.east);
        ...
        \node[block, fill=green!10, draw=green!40!black,
          ...
          (ca) at (0, 4.10) {Cross-Attention};

        % Encoder input arrow from the left
        \node[font=\tiny, text=green!60!black, anchor=east] at (-5.60, 4.10)
          {$K,V$ from encoder $\rightarrow$};
        \draw[arr, green!50!black] (-5.10, 4.10) -- (ca.west);

        \draw[arr] (\xA, 4.43) -- (\xA, 5.50);
        \draw[arr] (\xB, 4.43) -- (\xB, 5.50);

        % ======== ADD & NORM 2 ========
        \node[block, fill=yellow!15, draw=yellow!50!black,
              minimum width=4.8cm, font=\small, text=yellow!50!black]
          (an2) at (0, 5.80) {Add \& LN};

        % Residual 2
        \draw[gray!50, thin, dashed, rounded corners=3pt]
          (\xA, 3.40) -- (-5.00, 3.40) -- (-5.00, 5.80) -- (an2.west);
        \draw[gray!50, thin, dashed, rounded corners=3pt]
          (\xB, 3.40) -- (5.00, 3.40) -- (5.00, 5.80) -- (an2.east);

        \draw[arr] (\xA, 6.10) -- (\xA, 6.85);
        \draw[arr] (\xB, 6.10) -- (\xB, 6.85);

        % ======== FEED-FORWARD ========
        \node[block, fill=cyan!12, draw=cyan!40,
              minimum width=1.8cm, text=cyan!60!black]
          (ff1) at (\xA, 7.15) {Feed-Forward};
        \node[block, fill=cyan!12, draw=cyan!40,
              minimum width=1.8cm, text=cyan!60!black]
          (ff2) at (\xB, 7.15) {Feed-Forward};

        \draw[arr] (\xA, 7.48) -- (\xA, 8.20);
        \draw[arr] (\xB, 7.48) -- (\xB, 8.20);

        % ======== ADD & NORM 3 ========
        \node[block, fill=yellow!15, draw=yellow!50!black,
              minimum width=4.8cm, font=\small, text=yellow!50!black]
          (an3) at (0, 8.50) {Add \& LN};

        % Residual 3
        \draw[gray!50, thin, dashed, rounded corners=3pt]
          (\xA, 6.45) -- (-5.00, 6.45) -- (-5.00, 8.50) -- (an3.west);
        \draw[gray!50, thin, dashed, rounded corners=3pt]
          (\xB, 6.45) -- (5.00, 6.45) -- (5.00, 8.50) -- (an3.east);

        % Output
        \draw[arr] (\xA, 8.80) -- (\xA, 9.15);
        \draw[arr] (\xB, 8.80) -- (\xB, 9.15);

        \node[font=\tiny, text=blue!60!black, anchor=east] at (\xA-0.30, 9.35) {$o_1$};
        \foreach \ci in {0,...,3} {
          \pgfmathsetmacro{\cx}{\xA - 2*\cs + \ci*\cs + \cs/2}
          \node[acell, minimum width=\cs cm, minimum height=\cs cm,
                fill=blue!20, draw=blue!40] at (\cx, 9.35) {};
        }
        \node[font=\tiny, text=blue!60!black, anchor=east] at (\xB-0.30, 9.35) {$o_2$};
        \foreach \ci in {0,...,3} {
          \pgfmathsetmacro{\cx}{\xB - 2*\cs + \ci*\cs + \cs/2}
          \node[acell, minimum width=\cs cm, minimum height=\cs cm,
                fill=blue!20, draw=blue!40] at (\cx, 9.35) {};
        ...
        \draw[violet!40!black, rounded corners=8pt, line width=1pt]
          (-5.30, 0.40) rectangle (5.30, 9.10);
        \node[font=\small\bfseries, text=violet!40!black]
          at (0, 9.70) {Decoder Block};
...
\begin{frame}{Positional Encoding}
  Self-attention is \textbf{permutation equivariant} --- no notion of token order.
...
\begin{frame}{KV-Cache: Avoiding Redundant Computation}
  ...
  \begin{columns}[T]
    ...
    \begin{algorithm}[H]
      \caption{Autoregressive Generation with KV-Cache}
      \KwInput{prompt tokens $x_1, \ldots, x_P$; model with $L$ layers}
      \KwOutput{generated tokens $x_{P+1}, \ldots, x_{P+G}$}
      \BlankLine
      \tcp{Prefill: process prompt in parallel}
      \For{$\ell = 1, \ldots, L$}{
        $Q, K, V \leftarrow W^Q x,\; W^K x,\; W^V x$ \tcp*{all $P$ tokens}
        $K^\ell_{\textnormal{cache}} \leftarrow K$;\;
        $V^\ell_{\textnormal{cache}} \leftarrow V$ \;
        $X \leftarrow \textnormal{TransformerBlock}^\ell(Q, K, V, X)$ \;
      }
      \BlankLine
      \tcp{Decode: one token at a time}
      $t \leftarrow P$ \;
      \While{$x_t \neq $ eos \textbf{and} $t < P + G$}{
        $t \leftarrow t + 1$ \;
        \For{$\ell = 1, \ldots, L$}{
          $q_t, k_t, v_t \leftarrow W^Q x_t,\; W^K x_t,\; W^V x_t$ \tcp*{1 token}
          \textcolor{orange!80!black}{$K^\ell_{\textnormal{cache}} \leftarrow [K^\ell_{\textnormal{cache}};\; k_t]$} \tcp*{append key}
          \textcolor{cyan!60!black}{$V^\ell_{\textnormal{cache}} \leftarrow [V^\ell_{\textnormal{cache}};\; v_t]$} \tcp*{append value}
          $o_t \leftarrow \textnormal{Attn}(q_t, K^\ell_{\textnormal{cache}}, V^\ell_{\textnormal{cache}})$ \;
          $x_t \leftarrow \textnormal{FFN}(\textnormal{LN}(o_t + x_t))$ \;
        }
        $x_t \leftarrow \textnormal{sample}(\textnormal{softmax}(\textnormal{LMHead}(x_t)))$ \;
      }
    \end{algorithm}
    \end{minipage}
    }
    ...
    \begin{column}{0.48\textwidth}
      ...
      \begin{tikzpicture}[scale=0.82,
        ...
        % --- K row: cached + new ---
        \node[cachelbl, anchor=east] at (-0.15, 3.0) {$K$:};
        \foreach \i in {0,1,2} {
          \pgfmathsetmacro{\tx}{\i * 1.10 + 0.35}
          \node[tok, fill=kcol!50] at (\tx, 3.0) {$k_{\the\numexpr\i+1}$};
        }
        \node[tok, fill=kcol] at (3.65, 3.0) {$k_4$};
        \draw[decorate, decoration={brace, amplitude=3pt, mirror}, orange!60, thick]
          (-0.15, 2.70) -- (2.75, 2.70)
          node[midway, below=4pt, font=\tiny, orange!80!black] {cached};
        \node[font=\tiny, orange!70] at (4.5, 3.0) {new};

        % --- V row: cached + new ---
        \node[cachelbl, anchor=east] at (-0.15, 2.0) {$V$:};
        \foreach \i in {0,1,2} {
          \pgfmathsetmacro{\tx}{\i * 1.10 + 0.35}
          \node[tok, fill=vcol!50] at (\tx, 2.0) {$v_{\the\numexpr\i+1}$};
        }
        \node[tok, fill=vcol] at (3.65, 2.0) {$v_4$};
        \draw[decorate, decoration={brace, amplitude=3pt, mirror}, cyan!50, thick]
          (-0.15, 1.70) -- (2.75, 1.70)
          node[midway, below=4pt, font=\tiny, cyan!60!black] {cached};
        \node[font=\tiny, cyan!50!black] at (4.5, 2.0) {new};

        % --- Attention computation ---
        \draw[arr, thick] (2.2, 1.30) -- (2.2, 0.80);
        \node[draw=gray!50, fill=green!10, rounded corners=3pt,
              minimum width=3.6cm, minimum height=0.45cm, font=\scriptsize]
          at (2.2, 0.50) {$\text{softmax}\!\bigl(\frac{q_4 K_{1:4}^\top}{\sqrt{d_k}}\bigr) V_{1:4}$};

        % --- Output ---
        \draw[arr, thick] (2.2, 0.25) -- (2.2, -0.15);
        \node[tok, fill=zcol, font=\scriptsize] at (2.2, -0.40) {output $o_4$};
      \end{tikzpicture}
      \end{center}
    \end{column}
  \end{columns}
...
\begin{frame}{KV-Cache: Prefill vs.\ Decode}
  ...
  \begin{description}[leftmargin=1.5cm, style=nextline]
    \item[Prefill] Process the prompt as one batch (like training).
      Fill each layer's KV-cache with $K, V \in \mathbb{R}^{P \times d_k}$.
    \item[Decode] Generate tokens one by one.
      Each step: project \emph{one} token, append $k_t, v_t$ to cache, attend over full cache.
  \end{description}

  \vspace{0.3em}
  \begin{alertblock}{\small Prefill vs.\ Decode}
    \begin{itemize}
      \item Prefill is \textbf{compute-bound}: large matrix multiplies, high arithmetic intensity.
      \item Decode is \textbf{memory-bound}: reading KV-cache dominates; only 1 query vector per step.
    \end{itemize}
  \end{alertblock}

  \vspace{0.3em}
  \textbf{Cost per decode step:}
  \begin{description}[labelwidth=1.6cm, leftmargin=1.8cm, font=\small]
    \item[Na\"ive] $O(t \cdot d)$ to project all $t$ tokens into $Q,K,V$
    \item[KV-cache] $O(d)$ projection (1 token) $+$ $O(t \cdot d)$ attention
  \end{description}
...
\begin{frame}{KV-Cache: The Memory Problem}
  ...
  \vspace{0.3em}
  \textbf{Example} (LLaMA-2 70B, BF16, $T = 4096$):
  \begin{itemize}
  ...
  \begin{alertblock}{\small Approaches to Reduce KV-Cache}
    \begin{description}[labelwidth=3cm, leftmargin=3.2cm, font=\small]
      ...
      \item[Lower precision] KV-cache quantization (FP8, INT4)
    \end{description}
  \end{alertblock}
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.46\textwidth}
      ...
      \begin{tikzpicture}[scale=0.75,
        ...
        % --- MHA ---
        \node[font=\scriptsize\bfseries] at (1.4, 4.2) {MHA};
        \foreach \i in {0,...,3} {
          \pgfmathsetmacro{\xp}{\i * 0.9 + 0.1}
          \node[headbox, fill=qcol] (q\i) at (\xp, 3.5) {$Q_{\the\numexpr\i+1}$};
          \node[kvbox, fill=kcol] (k\i) at (\xp, 2.5) {$K_{\the\numexpr\i+1}$};
        ...
        % --- MQA ---
        \node[font=\scriptsize\bfseries] at (1.4, 0.7) {MQA};
        \foreach \i in {0,...,3} {
          \pgfmathsetmacro{\xp}{\i * 0.9 + 0.1}
          \node[headbox, fill=qcol] (mq\i) at (\xp, 0.0) {$Q_{\the\numexpr\i+1}$};
        }
        \node[kvbox, fill=kcol] (mk) at (1.4, -1.0) {$K$};
        \node[kvbox, fill=vcol] (mv) at (1.4, -1.8) {$V$};
        \foreach \i in {0,...,3} {
          \draw[arr] (mq\i) -- (mk);
        }
        \draw[arr] (mk) -- (mv);
...
\begin{frame}{Multi-head Latent Attention (MLA)}
  ...
  \begin{columns}[T]
    \begin{column}{0.52\textwidth}
      \textbf{Down-project} (KV compression):
      \[
        \vc^{KV}_t = W^{DKV} \vh_t \quad \in \mathbb{R}^{d_c}
      \]

      \textbf{Up-project} to reconstruct K,V:
      \[
        \vk^C_t = W^{UK}\, \vc^{KV}_t, \quad
        \vv_t = W^{UV}\, \vc^{KV}_t
      \]

      \textbf{Queries} (also compressed for training):
      \[
        \vc^{Q}_t = W^{DQ} \vh_t, \quad
        \vq^C_t = W^{UQ}\, \vc^{Q}_t
      \]

      \vspace{0.3em}
      \textbf{Key insight:} The KV information across all heads lies in a low-dimensional subspace.
      We cache the compressed representation and reconstruct on the fly.
    \end{column}
    \begin{column}{0.46\textwidth}
      \centering
      \begin{tikzpicture}[scale=0.80,
        ...
        % h_t
        \node[box, fill=gray!15] (ht) at (0, 4.5) {$\vh_t$};
...
\begin{frame}{MLA: Decoupled RoPE}
  ...
  \textbf{Problem:} RoPE applies position-dependent rotations $R_t$ to queries and keys.
  \[
    \text{score}_{t,s} = (\underbrace{R_t \vq^C_t}_{\text{pos-dependent}})^\top (\underbrace{R_s \vk^C_s}_{\text{pos-dependent}})
  \]
  Without RoPE, we could absorb the up-projections:
  ...
  \vspace{0.3em}
  \textbf{Solution: Decoupled RoPE keys.}
  ...
  \vspace{0.3em}
  \textbf{Cache per token per layer:} $\vc^{KV}_t \in \mathbb{R}^{d_c}$ and $\vk^R_t \in \mathbb{R}^{d_R}$
...
% -----------------------------------------------------------------------
\begin{frame}{Cross-Layer Attention \& KV-Cache Quantization}

  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      \textbf{Cross-Layer Attention (CLA)}~\cite{brandon2024cla}

      \vspace{0.2em}
      Adjacent layers \emph{share} the same KV-cache.
      ...
      \vspace{0.2em}
      \textbf{Reduction:} $c\times$ less KV-cache (across layers).

      \vspace{0.2em}
      {\footnotesize CLA is \emph{orthogonal} to GQA/MQA -- combine for multiplicative savings.}
    \end{column}
    \begin{column}{0.48\textwidth}
      \textbf{KV-Cache Quantization}~\cite{hooper2024kvquant}

      \vspace{0.2em}
      Store cached K,V in lower precision (FP8, INT4, INT2)
      instead of BF16.

      \vspace{0.2em}
      \textbf{Reduction:} $b_{\text{orig}} / b_q$ (e.g.\ BF16 $\to$ INT4 = $4\times$).

      \vspace{0.2em}
      \textbf{Techniques:}
      \begin{itemize}
        \item Per-channel quantization for keys (large channel outliers)
        \item Per-token quantization for values
        \item Non-uniform quantization via $k$-means centroids
        \item Dense-and-sparse: outliers stored separately in full precision
      \end{itemize}

      \vspace{0.2em}
      {\footnotesize Post-training, no retraining needed.
      FP8 KV-cache is standard in vLLM / TensorRT-LLM.}
    \end{column}
  \end{columns}
...
\begin{frame}{KV-Cache Reduction: Summary}
  ...
  \begin{tabular}{llcc}
    ...
    KV quantization & Lower precision storage & $2\, h\, d_k \cdot b_q/b$ & $b_q/b$ \\
    \bottomrule
  \end{tabular}
  }
  \end{center}
