...
\begin{frame}{Causal Self-Attention}
  ...
  \begin{columns}[T]
    \begin{column}{0.50\textwidth}
      \vspace{0.3em}
      \[
        s_{ij} = \begin{cases} q_i k_j^\top / \sqrt{d_k} & j \le i \\ -\infty & j > i \end{cases}
      \]
...
% -----------------------------------------------------------------------
\begin{frame}{Training vs.\ Inference: Same Features per Position}
  \begin{itemize}
    \item Embedding, FFN, LayerNorm act per-token: trivially identical at training and inference.
    \item Only the attention step at position $t$ needs to be compared.
  \end{itemize}

  \begin{columns}[T]
    \begin{column}{0.46\textwidth}
      \textbf{Training} (all $t$ in parallel):
      \[
        z_t \;=\; \mathrm{softmax}\!\left(\underbrace{\frac{q_t K^\top}{\sqrt{d_k}}}_{(s_1, \ldots, s_T)} + M_t\right) V
      \]
      with causal mask
      $M_{tj} = \begin{cases} 0 & j \le t \\ -\infty & j > t \end{cases}$.
    \end{column}
    \begin{column}{0.52\textwidth}
      \textbf{Inference} (step $t$, KV-cache holds $\{k_j, v_j\}_{j<t}$):
      \begin{align*}
        s_j &= q_t k_j^\top / \sqrt{d_k}, \qquad j = 1, \ldots, t \\[-1pt]
        a_j &= \exp(s_j) \,\Big/ \!\!\sum_{j'=1}^{t}\! \exp(s_{j'}) \\[-1pt]
        z_t &= \sum_{j=1}^{t} a_j\, v_j
      \end{align*}
    \end{column}
  \end{columns}
  ...
  \begin{block}{Equivalence}
    ...
    \[
      z_t^{\mathrm{train}}
      = \sum_{j=1}^{T} \frac{\exp(s_j + M_{tj})}{\sum_{j'=1}^{T} \exp(s_{j'} + M_{tj'})}\, v_j
      \;\stackrel{\exp(-\infty)=0}{=}\;
      \sum_{j=1}^{t} \frac{\exp(s_j)}{\sum_{j'=1}^{t} \exp(s_{j'})}\, v_j
      = z_t^{\mathrm{infer}}
    \]
    Masked entries vanish from both numerator and denominator, collapsing the training sum to the inference sum.
  \end{block}
\end{frame}
...
\begin{frame}{Encoder Block}
  ...
  \begin{columns}[T]
    \begin{column}{0.38\textwidth}
      ...
      \begin{align*}
        F &= \mathrm{FFN}(X') \\
          &= \mathrm{GELU}\!\bigl(X'\, \underbrace{W_1}_{d \times d_{\mathrm{ff}}}\bigr)\, \underbrace{W_2}_{d_{\mathrm{ff}} \times d} \\
...
\begin{frame}{Layer Normalization}
  \textbf{LayerNorm}~\cite{ba2016layernorm} normalizes each token's feature vector independently across the feature dimension $d$.
  ...
  \begin{columns}[T]
    \begin{column}{0.55\textwidth}
      ...
      \begin{align*}
        ...
        \mathrm{LN}(x)_i &= \gamma_i\, \hat{x}_i + \beta_i
      \end{align*}
      \begin{itemize}
        \item $\gamma, \beta \in \mathbb{R}^d$ are learned scale and shift.
        \item Applied per token, per layer: shape and statistics are independent of batch and sequence length.
      \end{itemize}
    \end{column}
    \begin{column}{0.42\textwidth}
      \begin{block}{Why LayerNorm in transformers?}
        \begin{itemize}
          \item BatchNorm depends on batch statistics; sequence lengths and padding make these noisy.
          \item LayerNorm is independent of batch and length: identical at train and test.
          \item Stabilizes residual streams whose magnitude grows with depth.
        \end{itemize}
      \end{block}
    \end{column}
  \end{columns}
  ...
  \begin{tikzpicture}[scale=1.0]
    ...
    \foreach \pos in {0,...,9} {
      ...
      \foreach \dimi in {0,...,15} {
        ...
        % freq = 10000^(2*halfdi/d) with d = \cols
        \pgfmathsetmacro{\freq}{exp(ln(10000)*2*\halfdi/\cols)}
