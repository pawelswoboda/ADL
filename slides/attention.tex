...
\begin{frame}{Token Embeddings}
  ...
  \begin{tikzpicture}[scale=0.95,
    ...
    \node[font=\small\bfseries, text=green!60!black] at (-1.30, 3.35) {$E$};
    \foreach \ri in {0,...,6} {
      \foreach \ci in {0,...,3} {
    ...
    \node[font=\tiny, gray] at (-1.30, 3.10) {\scriptsize$d$};
    \node[font=\tiny, gray, rotate=90, anchor=south] at (-1.80, 2.20) {\scriptsize$V$};
...
\begin{frame}{Self-Attention: Softmax \& Weighted Output}
  ...
  \begin{tikzpicture}[scale=0.88]
    ...
    % --- term 2: 0.10 × v2 ---
    \node[font=\small, text=green!60!black, anchor=east] at (3.50, 0) {$0.10\;\times$};
    \foreach \ci in {0,...,2} {
      \pgfmathsetmacro{\cx}{3.55 + \ci*0.36 + 0.18}
      \node[acell, minimum width=0.36cm, minimum height=0.36cm,
            fill=vcol!25, draw=cyan!25] at (\cx, 0) {};
    }
    \node[font=\small, text=cyan!40!black] at (4.09, -0.38) {$v_2$};
    \node[font=\tiny, text=green!50!black, anchor=east] at (3.50, -0.38) {$\alpha_{22}$};
...
\begin{frame}{Transformer Architecture: Encoder \& Decoder}
  ...
  \begin{center}
    \includegraphics[width=0.55\textwidth]{DL_WS202425/The_transformer_encoder_decoder_stack.png}
  \end{center}
...
\begin{frame}{KV-Cache: Prefill vs.\ Decode}
  ...
  \begin{alertblock}{\small Optimizations (later)}
    KV-cache memory can become very large.
    Techniques: GQA, paged attention, quantized caches, sliding-window attention.
  \end{alertblock}
