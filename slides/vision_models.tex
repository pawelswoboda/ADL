...
% -----------------------------------------------------------------------
\begin{frame}{DeiT: RandAugment \& Random Erasing}

  Two single-image augmentations applied independently to each example.

  \vspace{0.3em}
  \begin{columns}[T]
    \begin{column}{0.5\textwidth}
      \begin{center}
        \textbf{RandAugment}\\[0.2em]
        \begin{tikzpicture}[scale=0.55, every node/.style={scale=0.55}]
          ...
          \node[font=\tiny, above, align=center] at (2.6, 1) {Rotate $+$\\Solarize};
          \begin{scope}[shift={(3.2, 0)}]
            \begin{scope}
              \clip (0,0) rectangle (2,2);
              \begin{scope}[rotate around={12:(1,1)}]
                \visionSketchScene
              \end{scope}
              \fill[orange, opacity=0.35] (0,0) rectangle (2,2);
            \end{scope}
            \draw[thick] (0,0) rectangle (2,2);
          \end{scope}
        \end{tikzpicture}
    ...
    \begin{column}{0.5\textwidth}
      \begin{center}
        ...
        \begin{tikzpicture}[scale=0.55, every node/.style={scale=0.55}]
          ...
          \draw[thick] (0,0) rectangle (2,2);
          \draw[-Stealth, thick] (2.2, 1) -- (3.0, 1);
          \node[font=\tiny, above] at (2.6, 1) {erase};
          \begin{scope}[shift={(3.2, 0)}]
            \begin{scope}
              \clip (0,0) rectangle (2,2);
              \visionSketchScene
  ...
  \begin{columns}[T]
    \begin{column}{0.5\textwidth}
      {\footnotesize
      \cite{cubuk2020randaugment}: two hyperparameters $N$ ops, magnitude $M$.
      \begin{enumerate}\itemsep0pt
        \item Sample $N$ ops uniformly from a fixed pool of 14
        \item Apply each with shared magnitude $M$
      \end{enumerate}
      Pool: AutoContrast, Equalize, Invert, Solarize, Posterize, Contrast, Color, Brightness, Sharpness, Rotate, ShearX/Y, TranslateX/Y.\\
      DeiT: $N{=}2$, $M{=}9$.}
    \end{column}
    \begin{column}{0.5\textwidth}
      {\footnotesize
      \cite{zhong2020randomerasing}: with probability $p$, replace a random rectangle of pixels with random / mean / zero values.
      \begin{itemize}\itemsep0pt
        \item Area ratio $S_e \sim U(s_l, s_h)$
        \item Aspect ratio $r_e \sim U(r_l, r_h)$
        \item Position uniform in image
      \end{itemize}
      Encourages \textbf{occlusion robustness}: the model cannot rely on a single discriminative region.}
    \end{column}
  \end{columns}
\end{frame}

% -----------------------------------------------------------------------
\begin{frame}{DeiT: Mixup \& CutMix}

  Both create new training samples by combining \textbf{pairs} of images and labels.
      ...
      \begin{center}
        ...
        \begin{tikzpicture}[scale=0.45, every node/.style={scale=0.45}]
          ...
          \begin{scope}[shift={(6.0, 0)}]
            \begin{scope}
              \clip (0,0) rectangle (2,2);
              \begin{scope}[transparency group, opacity=0.55]
                \visionSketchScene
              \end{scope}
              \begin{scope}[transparency group, opacity=0.55]
                \visionSketchSceneB
        ...
        \begin{tikzpicture}[scale=0.45, every node/.style={scale=0.45}]
          ...
          \begin{scope}[shift={(3.0, 0)}]
            \begin{scope}
              ...
              \visionSketchSceneB
            \end{scope}
            \draw[very thick, dashed, red] (0.5, 0.55) rectangle (1.45, 1.4);
            \draw[thick] (0,0) rectangle (2,2);
          \end{scope}
          \node[font=\Large] at (5.5, 1) {$=$};
          \begin{scope}[shift={(6.0, 0)}]
            ...
            \end{scope}
            \begin{scope}
              \clip (0.5, 0.55) rectangle (1.45, 1.4);
              \visionSketchSceneB
            \end{scope}
            \draw[very thick, dashed, red] (0.5, 0.55) rectangle (1.45, 1.4);
            \draw[thick] (0,0) rectangle (2,2);
  ...
  \begin{columns}[T]
    \begin{column}{0.5\textwidth}
      {\footnotesize
      \cite{zhang2018mixup}: linear blend
      \[
        \tilde{x} = \lambda\, x_1 + (1{-}\lambda)\, x_2, \quad
        \tilde{y} = \lambda\, y_1 + (1{-}\lambda)\, y_2
      \]
      $\lambda \sim \mathrm{Beta}(\alpha, \alpha)$, DeiT uses $\alpha{=}0.8$.\\
      Encourages \textbf{linear behavior} between examples and smoother decision boundaries.}
    \end{column}
    \begin{column}{0.5\textwidth}
      {\footnotesize
      \cite{yun2019cutmix}: paste a random box
      \[
        \tilde{x} = (1{-}M) \odot x_1 + M \odot x_2, \quad
        \tilde{y} = \lambda y_1 + (1{-}\lambda) y_2
      \]
      $\lambda = 1 - |B|/(HW)$ is the area outside the patch.\\
      No unrealistic blended pixels; encourages spatial attention and improves localization.}
    \end{column}
  \end{columns}
\end{frame}
  ...
  \begin{columns}[T]
    \begin{column}{0.55\textwidth}
      ...
      \begin{itemize}\itemsep0pt
        ...
        \item Positional embeddings added to \textbf{all} tokens, so the decoder knows where each one belongs
      \end{itemize}
