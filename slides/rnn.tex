...
% -----------------------------------------------------------------------
\begin{frame}{Two Ways to Process a Sequence}
  How should a model access the past when predicting $y_t$ from $x_1, \ldots, x_t$?

  \vspace{0.5em}
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      \textbf{Recurrent state} (RNN, LSTM, SSM)
      \[
        h_t = f_\theta(h_{t-1}, x_t), \qquad h_t \in \R^n
      \]
      \begin{itemize}
        \item Compress the entire history into a \textbf{fixed-size vector} $h_t$
        \item Past is accessed \emph{indirectly}, through whatever the state retained
        \item Per-step cost $\mathcal{O}(n)$, independent of $t$
        \item Memory $\mathcal{O}(n)$, independent of $T$
      \end{itemize}
    \end{column}
    \begin{column}{0.48\textwidth}
      \textbf{Attention} (Transformer)
      \[
        y_t = \sum_{s \le t} \alpha_{ts}\, v_s, \quad \alpha_{ts} \propto e^{q_t^\top k_s}
      \]
      \begin{itemize}
        \item Keep \textbf{all past tokens} $(k_s, v_s)$ and look them up on demand
        \item Past is accessed \emph{directly}, with no lossy compression
        \item Per-step cost $\mathcal{O}(t)$, grows with context
        \item Memory $\mathcal{O}(T)$, grows with context
      \end{itemize}
    \end{column}
  \end{columns}
  ...
  \begin{block}{The core trade-off}
    \begin{itemize}
      \item \textbf{Recurrence:} $\mathcal{O}(1)$ memory and streaming inference, but an \emph{information bottleneck}: the state must decide ahead of time what to keep
      \item \textbf{Attention:} $\mathcal{O}(T)$ memory and $\mathcal{O}(T)$ per-step cost, but can revisit any past token \emph{exactly}
    \end{itemize}
  \end{block}
\end{frame}
  ...
  \begin{columns}[T]
    \begin{column}{0.50\textwidth}
      ...
      \begin{tikzpicture}[
        ...
        % Backward gradient arrows: thickness shrinks to depict vanishing
        \draw[red, -{Stealth[length=4pt]}, line width=1.6pt] (h3.south) to[bend left=40] (h2.south);
        \draw[red, -{Stealth[length=4pt]}, line width=1.0pt] (h2.south) to[bend left=40] (h1.south);
        \draw[red, -{Stealth[length=4pt]}, line width=0.55pt] (h1.south) to[bend left=40] (h0.south);
        \draw[red, -{Stealth[length=4pt]}, line width=0.2pt] (h0.south) to[bend left=40] (h_init.south);
        \node[red, font=\scriptsize] at (3.0, 2.7) {gradient $\frac{\partial \mathcal{L}}{\partial h_t}$ shrinks (or blows up) with $k$};
        \draw[red, -{Stealth[length=3pt]}, line width=0.4pt] (3.0, 2.5) -- (3.0, 1.0);
      \end{tikzpicture}
      \end{center}

      \vspace{0.2em}
      The state $h_t$ is a \textbf{lossy compression} of $x_1, \ldots, x_t$ into a fixed-dimensional vector.
    \end{column}
    \begin{column}{0.48\textwidth}
      \textbf{Vanishing / exploding gradients}

      Backpropagation through time chains Jacobians of the recurrence:
      \[
        \frac{\partial h_t}{\partial h_{t-k}} \,=\, \prod_{i=1}^{k} \underbrace{\frac{\partial h_{t-i+1}}{\partial h_{t-i}}}_{=:\, J_i}
      \]
      Let $\sigma$ be a typical spectral radius of $J_i$:
      \begin{itemize}
        \item $\sigma < 1$: norm decays as $\sigma^k$ $\Rightarrow$ \textbf{vanishing}, long-range signal lost
        \item $\sigma > 1$: norm grows as $\sigma^k$ $\Rightarrow$ \textbf{exploding}, unstable updates
      \end{itemize}
      The stable regime $\sigma \approx 1$ is a knife-edge: hard to maintain across many steps without architectural help (gating, orthogonal $W$, residual paths).
  ...
  \begin{columns}
    ...
    \begin{column}{0.5\textwidth}
      ...
      \begin{tikzpicture}[
        ...
        % --- Layer 1 ---
        \node[init] (a10) at (0, 0) {$a^{[1]\la 0\ra}$};
        \foreach \t in {1,2,3} {
          \node[cell] (c1\t) at (\t*\dx, 0) {};
        }
        \draw[arr] (a10) -- (c11);
        ...
        % --- Layer 2 ---
        \node[init] (a20) at (0, \dy) {$a^{[2]\la 0\ra}$};
        \foreach \t in {1,2,3} {
        ...
        % --- Layer k (top) ---
        \def\ky{2.5*\dy}
        \node[init] (ak0) at (0, \ky) {$a^{[k]\la 0\ra}$};
        \foreach \t in {1,2,3} {
        ...
        % Time and depth axes
        \draw[->, thick, gray] (-0.6, -2.3) -- (3.5*\dx, -2.3)
          node[midway, below, font=\normalsize, text=gray] {time};
        \draw[->, thick, gray] (-1.6, -0.5) -- (-1.6, \ky+0.5)
          node[midway, left, font=\normalsize, text=gray, rotate=90, anchor=south] {depth};
      \end{tikzpicture}
      \end{center}
    \end{column}
  \end{columns}
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.47\textwidth}
      ...
      \begin{itemize}
        ...
        \item \textbf{Long-range dependencies:} hidden state must compress all history, so information decays
...
\newcommand{\grudiagram}[1]{%
  \begin{tikzpicture}[xscale=0.38, yscale=0.32, every node/.style={xscale=0.38, yscale=0.32},
    ...
    %
    % === State line (top) ===
    % Left segment up to h_{t-1} bifurcation at x=-0.5 — part of input
    \draw[line, opacity=\opacIn] (-3, \stateY) -- (-0.5, \stateY);
    % From bifurcation into zmul — part of combine
    \draw[arr, opacity=\opacComb] (-0.5, \stateY) -- (-0.1, \stateY);
    \draw[arr, opacity=\opacComb] (0.7, \stateY) -- (4.6, \stateY);
    \draw[arr, opacity=\opacComb] (5.4, \stateY) -- (8, \stateY);
    %
    % === Labels ===
    \node[font=\Large, above, opacity=\opacIn] at (-2.5, \stateY) {$h_{t-1}$};
    ...
    \draw[line, opacity=\opacIn] (-0.5, \stateY) -- (-0.5, 0.5);
    %
    ...
    %
    % === Gate connections ===
    % z_t -> zmul on state line (1-z_t) — used by combine and update
    \draw[arr, opacity=\opacComb] (zt.north) -- (zmul.south);
    \draw[arr, opacity=\opacZt] (zt.north) -- (zmul.south);
    \node[font=\Large, right, opacity=\opacComb] at (0.6, 6.0) {$1{-}z_t$};
    \node[font=\Large, right, opacity=\opacZt] at (0.6, 6.0) {$1{-}z_t$};
    %
    % r_t -> rmul — used by reset and candidate (drawn twice; highest opacity wins)
    \draw[arr, opacity=\opacRt] (rt.east) -- (rmul.west);
    \draw[arr, opacity=\opacCand] (rt.east) -- (rmul.west);
    \node[font=\Large, above, opacity=\opacRt] at (3.0, \gateY) {$r_t$};
    \node[font=\Large, above, opacity=\opacCand] at (3.0, \gateY) {$r_t$};
    %
    % rmul -> tanh — part of candidate
    \draw[arr, opacity=\opacCand] (rmul.east) -- (htilde.west);
    %
    % z_t -> ztmul — used by combine and update
    \draw[arr, opacity=\opacComb] (0.3, 4.5) -| ($(ztmul.south) + (-0.15, 0)$);
    \draw[arr, opacity=\opacZt] (0.3, 4.5) -| ($(ztmul.south) + (-0.15, 0)$);
    \node[font=\Large, above, opacity=\opacComb] at (2.5, 4.5) {$z_t$};
    \node[font=\Large, above, opacity=\opacZt] at (2.5, 4.5) {$z_t$};
    %
    ...
    \node[font=\Large, above, opacity=\opacCand] at (5.5, 4.5) {$\tilde{h}_t$};
    %
...
\begin{frame}{GRU vs.\ LSTM}
  ...
  \begin{tabular}{lcc}
    ...
    \multicolumn{3}{p{9cm}}{%
      \begin{itemize}
        \item GRU: fewer parameters $\Rightarrow$ faster training, smaller memory footprint
        \item LSTM: extra cell state and output gate give more capacity in principle
        \item Empirically often comparable~\cite{chung2014empirical}; LSTM tends to edge ahead on longer or more complex sequences, GRU on simpler ones~\cite{cahuantzi2021lstmgru}
  ...
  \begin{block}{Historical note}
    LSTMs (1997) and GRUs (2014) were the dominant sequence models before Transformers (2017). They remain relevant for streaming/real-time applications due to $\mathcal{O}(1)$ per-step computation.
  \end{block}
\end{frame}
