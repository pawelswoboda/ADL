...
\begin{frame}{SSD vs.\ Attention vs.\ SSM}
  ...
  \begin{tabular}{lccc}
    ...
    Training FLOPs       & $T^2 N$                 & $TNP$                   & $\bm{TNP}$ \\
    Inference FLOPs      & $TN$                    & $NP$                    & $\bm{NP}$ \\
    Naive memory         & $T^2$                   & $TNP$                   & $\bm{TN}$ \\
    \bottomrule
  \end{tabular}
  \end{center}
...
\begin{frame}{The Chunked SSD Algorithm}
  ...
  \begin{itemize}\itemsep1pt
    \item \emph{Linear / SSM recurrence mode} ($h_t = a_t h_{t-1} + B_t x_t^\top$): $O(TNP)$ FLOPs, but sequential and no matmuls (idles tensor cores).
  ...
  \begin{itemize}\itemsep1pt
    ...
    \item \textbf{Inter-chunk} (linear mode):
      ...
      \end{itemize}
    \item \textbf{Combine:} $Y = Y_{\text{intra}} + Y_{\text{inter}}$.
  \end{itemize}
...
\begin{frame}[fragile]{Reference Implementation: the \texttt{segsum} Helper}
    ...
    \begin{itemize}
        \item Source: \texttt{ssd\_minimal\_discrete} in the official Mamba-2 repo (\href{https://github.com/state-spaces/mamba/blob/main/mamba_ssm/modules/ssd_minimal.py}{\texttt{mamba\_ssm/modules/ssd\_minimal.py}})
