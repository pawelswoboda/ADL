...
\begin{frame}{RoPE: Why Long-Range Decay?}
  ...
  The RoPE score between positions $m$ and $n$ is a sum over dimension pairs:
  \[
    \text{score}(m,n) = \sum_{i=0}^{d/2-1} \underbrace{a_i}_{\text{content}} \cos(\Delta\,\theta_i) + \underbrace{b_i}_{\text{content}} \sin(\Delta\,\theta_i), \qquad \Delta = m{-}n
  \]
  where $a_i, b_i$ depend on $\vq, \vk$ and $\theta_i = \text{base}^{-2i/d}$ spans a huge frequency range.

  \vspace{0.3em}
  \begin{columns}[T]
    \begin{column}{0.55\textwidth}
      \textbf{At $\Delta = 0$:} All $\cos(0) = 1$, $\sin(0) = 0$.
      Score equals the unrotated dot product $\vq^\top \vk$. Full signal.

      \vspace{0.3em}
      \textbf{At increasing $|\Delta|$:}
      Each pair oscillates at its own frequency.
      The phases spread across the unit circle and \textbf{destructively interfere}:
      \begin{itemize}
        \item Small $|\Delta|$: Only high-freq pairs (small $i$) cancel. Low-freq pairs still coherent. Score slightly reduced.
        \item Large $|\Delta|$: Nearly all pairs oscillate incoherently. Only the slowest pairs contribute. Score near zero.
      \end{itemize}

      \vspace{0.3em}
      This is the \textbf{Riemann-Lebesgue effect}:
      a sum of oscillating terms at spread-out frequencies vanishes as the common phase shift grows.
    \end{column}
    \begin{column}{0.43\textwidth}
      \centering
      \textbf{Phasor view} at large $|\Delta|$:
      ...
      \begin{tikzpicture}[scale=0.72]
        ...
        \node[font=\scriptsize, anchor=west] at (0.35, 0.15) {$\sum$};

        \node[font=\scriptsize, gray, anchor=north] at (0, -2.2) {Phases spread $\Rightarrow$ sum $\approx 0$};
      \end{tikzpicture}

      \vspace{0.5em}
      \textbf{At $\Delta = 0$} all phasors point right ($e^{j \cdot 0} = 1$):
      ...
      \begin{tikzpicture}[scale=0.72]
        ...
        \node[font=\scriptsize, anchor=west] at (1.85, 0) {$\sum$};

        \node[font=\scriptsize, gray, anchor=north] at (0, -2.2) {All aligned $\Rightarrow$ max score};
      \end{tikzpicture}
    \end{column}
  \end{columns}
\end{frame}
