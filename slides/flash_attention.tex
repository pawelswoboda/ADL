...
\begin{frame}{HBM vs.\ SRAM}
  ...
  \begin{columns}
    \begin{column}{0.55\textwidth}
      ...
      \begin{tabular}{lcc}
        \toprule
        & \textbf{SRAM} & \textbf{HBM} \\
        \midrule
        Size   & $\sim$20\,MB   & $\sim$40--80\,GB \\
        Bandwidth & $\sim$19\,TB/s & $\sim$1.5--3\,TB/s \\
        Latency   & Low            & High \\
        \bottomrule
      \end{tabular}
      \end{center}
      ...
      \vspace{0.5em}
      \begin{itemize}
        \item SRAM is on-chip (shared memory), fast but small
        \item HBM is off-chip, large but slow
        \item Standard attention materializes $S, P \in \mathbb{R}^{T \times T}$ in HBM
      \end{itemize}
    \end{column}
    \begin{column}{0.42\textwidth}
      \centering
...
\begin{frame}{Standard Attention: Memory Access Pattern}
  ...
  \begin{columns}
    \begin{column}{0.55\textwidth}
      ...
      \begin{enumerate}
        \item Compute $S = QK^\top$ \quad $\to$ write $S$ to HBM
        \item Compute $P = \text{softmax}(S)$ \quad $\to$ read $S$, write $P$ to HBM
        \item Compute $O = PV$ \quad $\to$ read $P$ from HBM
      \end{enumerate}
