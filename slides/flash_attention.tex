...
\begin{frame}{The Problem with Standard Attention}
  ...
  \begin{columns}
    ...
    \begin{column}{0.42\textwidth}
      \centering
      \includegraphics[width=\linewidth]{images/flash_attention_time_ablation.png}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{HBM vs.\ SRAM}
  ...
  \begin{columns}
    ...
    \begin{column}{0.42\textwidth}
      \centering
      \includegraphics[width=\linewidth]{images/hbm_vs_sram.png}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{Recomputing $P$ from the Stored Log-Sum-Exp $L$}
  ...
  The backward pass needs $P$ but we did not store it. Instead, the forward stored a single scalar per row:
  \[
    L_i = m_i + \log(\ell_i), \qquad
    m_i = \max_j S_{ij}, \quad \ell_i = \sum_j e^{S_{ij} - m_i}
  \]

  \vspace{0.3em}
  \textbf{Identity used in the backward:}
  \vspace{-0.3em}
  \[
    P_{ij} = \frac{e^{S_{ij} - m_i}}{\ell_i}
           = \frac{e^{S_{ij} - m_i}}{e^{\log \ell_i}}
           = e^{S_{ij} - m_i - \log \ell_i}
           = e^{S_{ij} - L_i}
  \]

  \vspace{-0.3em}
  So in the backward kernel we simply recompute $S_{ij} = Q_i K_j^\top$ block-wise and apply $P_{ij} = \exp(S_{ij} - L_i)$.

  \vspace{0.4em}
  \begin{itemize}
    \item \textbf{One scalar per row, not two.} Storing $L_i$ instead of $(m_i, \ell_i)$ halves the saved auxiliary memory and fuses the normalization into a single $\exp$
    \item \textbf{Numerical stability is preserved.} Since $L_i \geq m_i \geq S_{ij}$, the exponent $S_{ij} - L_i \leq 0$, so $\exp(\cdot) \in (0,1]$, no overflow
    \item \textbf{Memory vs.\ compute trade-off.} We pay one extra $QK^\top$ block-matmul per backward, but avoid materializing the $T \times T$ matrix $P$ in HBM
    \item \textbf{Only $L$ couples forward and backward.} $L$ is the sole forward-pass artifact (besides $O$) carried into the backward, hence the line ``$L_i \gets m_i + \log(\ell_i)$'' marked \emph{Trick 2} in the forward algorithm
  \end{itemize}
\end{frame}
