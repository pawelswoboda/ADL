...
\begin{frame}{Mixed Precision Training}
  ...
  \begin{tikzpicture}[font=\small, scale=0.88, every node/.style={transform shape},
    ...
    % === Left: Storage ===
    \node[fp32] (mw) at (0, 6.7) {Master weights $\theta$};
    \node[lbl, above=0pt of mw] {float32};

    % Cast down
    \node[cast] (castdown) at (0, 5.7) {cast to bf16};
    \draw[arr] (mw) -- (castdown);

    % bf16 weights copy
    \node[bf16] (wbf) at (0, 4.8) {Weights $\hat{\theta}$};
    \node[lbl, right=2pt of wbf] {bfloat16};
    \draw[arr] (castdown) -- (wbf);

    % Forward
    \node[op] (fwd) at (0, 3.7) {Forward pass};
    \node[lbl, above right=1pt and 2pt of fwd] {bf16 compute};
    \draw[arr] (wbf) -- (fwd);

    % Input
    \node[bf16, minimum width=1.8cm] (input) at (-3.5, 3.7) {Input $x$};
    \node[lbl, above=0pt of input] {bf16};
    \draw[arr] (input) -- (fwd);

    % Loss (fp32)
    \node[fp32, minimum width=1.8cm] (loss) at (3.5, 3.7) {Loss $\mathcal{L}$};
    \node[lbl, above=0pt of loss] {float32};
    \draw[arr] (fwd) -- (loss);

    % Activations stored
    \node[bf16, minimum width=1.8cm] (act) at (-3.5, 2.6) {Activations};
    \node[lbl, above=0pt of act] {bf16};
    \draw[arr, dashed, green!50!black] (fwd) -- (act);

    % Backward
    \node[op] (bwd) at (0, 2.6) {Backward pass};
    \node[lbl, above=0pt of bwd] {bf16 compute};
    \draw[arr] (loss) |- (bwd);
    \draw[arr, dashed, green!50!black] (act) -- (bwd);

    % Gradients in fp32
    \node[fp32] (grad) at (0, 1.5) {Gradients $\nabla_\theta \mathcal{L}$};
  ...
  \begin{columns}
    ...
    \begin{column}{0.44\textwidth}
      \begin{tikzpicture}[font=\small, scale=0.82, every node/.style={transform shape},
        ...
        \draw[arr] (clip)  -- (opt);
        % inf/nan branch
        \node[font=\tiny, red, align=center] (skip) at (3.2, 1.95) {inf/nan?\\skip \& reduce $S$};
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.48\textwidth}
      \begin{tikzpicture}[font=\small, scale=0.82, every node/.style={transform shape},
        ...
        % Loop back
        \draw[arr, dashed, gray] (upd.south) -- ++(0,-0.4) -| (-1.1, 5.5) -- (g0.west);
      \end{tikzpicture}
    \end{column}
  \end{columns}
\end{frame}
