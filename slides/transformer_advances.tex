...
% -----------------------------------------------------------------------
\begin{frame}{KV-Cache: Compute and Memory}

  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      {\footnotesize Projections: $Q\!=\!XW_Q$, $K\!=\!XW_K$, $V\!=\!XW_V$, output $W_O$.}

      \textbf{Prefill} (compute-bound):
      \begin{itemize}
        \item Large matmuls, high arithmetic intensity
        \item $O(T \cdot d^2)$ proj.\ $+$ $O(T^2 \cdot d)$ attn.
      \end{itemize}

      \vspace{0.4em}
      \textbf{Decode} (memory-bound):
      \begin{itemize}
        \item Only new token projected; past $K,V$ from cache
        \item $O(d^2)$ proj.\ $+$ $O(t \cdot d)$ attn.\ per step
      \end{itemize}
    \end{column}
    \begin{column}{0.50\textwidth}
      \textbf{Cache size:}
      \[
        2 \cdot L \cdot h \cdot T \cdot d_k \cdot b \text{ bytes}
      \]
      {\footnotesize $L$ layers, $h$ heads, $T$ seq.\ length, $d_k$ head dim, $b$ bytes/elem.}

      \vspace{0.4em}
      \textbf{Example} (LLaMA-2 70B dims, MHA, BF16):\\
      $L{=}80$, $h{=}64$, $d_k{=}128$, $b{=}2$
      \begin{itemize}
        \item $T{=}4$k: $\approx 10.7$\,GB / sequence
        \item $T{=}128$k: $\approx 335$\,GB -- larger than the model!
        \item LLaMA-2 70B uses GQA ($g{=}8$), $8\times$ smaller.
      \end{itemize}
    \end{column}
  \end{columns}
...
\begin{frame}{QK-Norm~\cite{henry2020qknorm}}
  ...
  \textbf{Problem:} During training, query and key norms can grow too much.
...
% -----------------------------------------------------------------------
\begin{frame}{Normalization Placement: Pre-LN vs.\ Peri-LN~\cite{he2025periln}}
  ...
  \begin{columns}[T]
    \begin{column}{0.50\textwidth}
      \begin{center}\textbf{Pre-LN} (GPT-2, LLaMA)\end{center}
      \vspace{-0.3em}
      \[
        \begin{aligned}
      ...
      \begin{tikzpicture}[
        ...
        box/.style={draw, rounded corners=3pt, minimum width=1.5cm, minimum height=0.38cm, font=\scriptsize, align=center, inner sep=2pt},
        ...
        \node[addcirc] (add2) at (0, 3.7) {$+$};
        \draw[arr] (0,-0.2) -- (n1); \draw[arr] (n1) -- (a1); \draw[arr] (a1) -- (add1);
        \draw[arr] (add1) -- (n2); \draw[arr] (n2) -- (m1); \draw[arr] (m1) -- (add2);
        \draw[gray!50, thin, dashed, rounded corners=2pt] (-0.4,-0.1) -- (-1.2,-0.1) -- (-1.2,1.7) -- (add1.west);
        \draw[gray!50, thin, dashed, rounded corners=2pt] (-0.4,1.9) -- (-1.2,1.9) -- (-1.2,3.7) -- (add2.west);
      \end{tikzpicture}
      \end{center}
      ...
      \begin{itemize}
        ...
        \item Prone to ``massive activations'': hidden state norms grow across layers
      \end{itemize}}
    \end{column}
    \begin{column}{0.50\textwidth}
      \begin{center}\textbf{Peri-LN} (Gemma 2/3, OLMo 2)\end{center}
      \vspace{-0.3em}
      \[
        \begin{aligned}
      ...
      \begin{tikzpicture}[
        ...
        \node[norm] (pn1) at (0, 0.3) {Norm};
        \node[attn] (pa1) at (0, 1.0) {Attn};
        \node[norm, draw=red!60, thick] (pn2) at (0, 1.7) {Norm};
        \node[addcirc] (padd1) at (0, 2.4) {$+$};
        \node[norm] (pn3) at (0, 3.0) {Norm};
        \node[mlp] (pm1) at (0, 3.7) {MLP};
        \draw[arr] (0,-0.2) -- (pn1); \draw[arr] (pn1) -- (pa1); \draw[arr] (pa1) -- (pn2);
        \draw[arr] (pn2) -- (padd1); \draw[arr] (padd1) -- (pn3); \draw[arr] (pn3) -- (pm1);
        \draw[gray!50, thin, dashed, rounded corners=2pt] (-0.4,-0.1) -- (-1.2,-0.1) -- (-1.2,2.4) -- (padd1.west);
        \node[font=\tiny, red!60!black, anchor=west] at (0.85, 1.7) {extra norm};
      \end{tikzpicture}
      \end{center}
      ...
      {\footnotesize
      \begin{itemize}
        \item Wraps each sublayer with norm on both sides
...
\begin{frame}{SwiGLU Activation~\cite{shazeer2020glu}}
  ...
  \vspace{0.3em}
  \begin{block}{Why gating helps}
    Both $xW_{\text{gate}}$ and $xW_{\text{up}}$ are learned functions of $x$, so SwiGLU performs a \emph{data-dependent multiplicative interaction} between two features of the input. A plain MLP is strictly additive between projections and cannot express this. Used in LLaMA 1/2/3, Mistral, Gemma, PaLM.
  \end{block}
\end{frame}
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.48\textwidth}
      ...
      \begin{itemize}
        ...
        \item Gated attention (per-head sigmoid gate)
